...
function calculateCost() {
	 ...
	 for (var i = 0; i<incomeArray.length; i++) {
	 	...
	 	function CalculateSavings(){
	 		var savings= Income - Cost;
	 		console.log(`Total savings is ${savings}`);
	 		document.querySelector('#displaySavings').innerHTML=`Total Saving: $${savings}.00`;
	 		Saving=savings;
	 	}
...
function allocationSavings() {
	...
	allocateamountCell.innerHTML= allocateAmount;
	 ...
	 for (var i = 0; i<allocateArray.length; i++) {
	 	...
	 	var calcAllocateCell=document.getElementById("allocateTable"), minusAllocationValue=Saving;
	 	...
	 	document.querySelector("#displayAllocationResultId").innerHTML= `Saving left: ${ minusAllocationValue}`;
